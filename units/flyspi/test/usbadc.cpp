...
TEST(USBADC, IDontKnowWhatItDoes)
	{
		...
		if(myusb.read(0x100,result)){
			printf("read failed\n");ASSERT_EQ(0, myusb.status); return;;
		}else printf("Read1: %02x ", result);								
		if(myusb.read(0x101,result)){
			printf("read failed\n");ASSERT_EQ(0, myusb.status); return;;
		}else printf("Read2: %02x\n", result);								
	  ...
	  cout<<"Temperature: "<<tempi<<" "<<temp<<endl;
		...
		//test adc
		ocpw(myusb,0x3008,0x04); //start clock
		ocpw(myusb,0x3008,0xc4); //release pdn and reset for adc, no start signal yet
		ocpw(myusb,0x1c00,0x10); //software reset
		ocpw(myusb,0x1c01,0x4); //enable lvds interface, disable clock
		ocpw(myusb,0x1c00|(0xe<<3)|4,0xc); //lowest power lvds
		ocpw(myusb,0x1c00|(0x4<<3)|4,0x0); //clock edge
		ocpw(myusb,0x1c00|(0xb<<3)|5,0x54); //custom pattern 0x555 ->datablatt wrong!!!
		ocpw(myusb,0x1c00|(0xc<<3)  ,0x02); //custom pattern 0x555 ->datablatt wrong!!!
		...
		bbuf[12]=__builtin_bswap32(0x80003006);bbuf[13]=__builtin_bswap32(0);
		bbuf[14]=__builtin_bswap32(0x80003007);bbuf[15]=__builtin_bswap32(128);//end address
		bbuf[16]=__builtin_bswap32(0x80003008);bbuf[17]=__builtin_bswap32(0xc5);//start
 	  bbuf[18]=__builtin_bswap32(0x80003008);bbuf[19]=__builtin_bswap32(0xc4);//start off
